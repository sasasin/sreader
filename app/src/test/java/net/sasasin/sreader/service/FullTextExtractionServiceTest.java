...
class FullTextExtractionServiceTest {
	...
	void extractsTextWithXpathRule() throws Exception {
		...
		FullTextExtractionService service = new FullTextExtractionService(
				mock(ContentHeaderRepository.class),
				mock(ContentFullTextRepository.class),
				rules,
				http);
		ContentHeader header = new ContentHeader("id", "feed", "https://example.test/articles/1", "title", null);
		when(http.get(URI.create(header.url()))).thenReturn(new HttpFetchService.FetchedResource(
				URI.create(header.url()),
				"<html><body><article><h1>Hello</h1><p>World</p></article><nav>skip</nav></body></html>"));
		when(rules.findBestRule(header.url())).thenReturn(Optional.of(new ExtractRule("rule", "https://example.test/articles/", "//article")));
	...
	void fallsBackToBodyTextWhenNoRuleMatches() throws Exception {
		...
		FullTextExtractionService service = new FullTextExtractionService(
				...
				rules,
				http);
		ContentHeader header = new ContentHeader("id", "feed", "https://example.test/no-rule", "title", null);
		when(http.get(URI.create(header.url()))).thenReturn(new HttpFetchService.FetchedResource(
				URI.create(header.url()),
