    ...
    for (ImportFeed feed : feeds) {
      ...
      if (FeedStatus.ACTIVE.value().equals(current.status())
        ...
        if (!methodChanged) {
          ...
          counters.updated++;
          if (!options.dryRun()) {
            feedUrlRepository.updateFullTextMethod(feed.url(), feedMethod);
          }
        }
        ...
        boolean metadataChanged =
            !java.util.Objects.equals(current.unsubscribeReason(), feed.unsubscribeReason())
                || !java.util.Objects.equals(current.unsubscribedAt(), feed.unsubscribedAt())
    ...
    for (int i = 0; i < feedTables.size(); i++) {
      ...
      if (rawUrl == null) {
        ...
        try {
          ...
          if (!seenUrls.add(url)) {
            errors.add(
                "feeds[" + index + "].url duplicates another feed after normalization: " + url);
          }
      ...
      String rawUnsubscribedAt =
          stringValue(table.get("unsubscribed_at"), "feeds[" + index + "].unsubscribed_at", errors);
      ...
      String rawFullTextMethod =
          stringValue(
              table.get("full_text_method"), "feeds[" + index + "].full_text_method", errors);
      ...
      if (schemaVersion == 1) {
        if (rawFullTextMethod == null) {
          ...
          try {
            fullTextMethod = FullTextMethod.fromValue(rawFullTextMethod).value();
          } catch (IllegalArgumentException e) {
            errors.add("feeds[" + index + "].full_text_method is invalid: " + rawFullTextMethod);
            fullTextMethod = FullTextMethod.HTTP.value();
          }
        }
      } else {
      ...
      if (url != null) {
        feeds.add(new ImportFeed(index, url, status, reason, unsubscribedAt, note, fullTextMethod));
      }
    }
