...
public class FullTextExtractionService {
	...
	public FullTextExtractionService(ContentHeaderRepository contentHeaderRepository,
	...
	public int extractPending(int limit) {
		...
		for (ContentHeader header : headers) {
			try {
				...
				ContentFullText content = new ContentFullText(HashIds.md5(header.url()), header.id(), fullText);
	...
	public String extract(ContentHeader header) throws IOException, InterruptedException {
		...
		return extractRuleService.findBestRule(header.url())
