...
Flyway + PostgreSQL 18.x の標準的なアプリケーションとして作り直しました。
...
- Spring Boot: 3.5.7
- jOOQ: 3.21.6
- Flyway runtime: Spring Boot managed
- Flyway image: `redgate/flyway:12.9.0`
...
場合は実行しないでください。PostgreSQL 18 の公式 Docker image は
`/var/lib/postgresql` に volume を mount する構成です。17.x 以前の volume を
再利用する場合は、volume 削除または `pg_upgrade` による移行を検討してください。
